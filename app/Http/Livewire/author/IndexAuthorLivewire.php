...
class IndexAuthorLivewire extends Component
{
    ...
    {
        $idauthor = Crypt::decrypt($idauthor);
        $findbook = Book::where('id_author', $idauthor)->get('id');
        dd($findbook);
        if ($findbook != null) {
            $finborrow = Borrow::where('id_book', collect($findbook)->toArray())->get('id');
            if ($finborrow != null) {
                DetailBorrow::whereIn('id_book', collect($finborrow)->toArray())->truncate();
                Borrow::destroy(collect($finborrow)->toArray());
            }
            Book::destroy(collect($findbook)->toArray());
        }
